...
export const SchematicToolbar: React.FC<SchematicToolbarProps> = ({
    ...
    return (
        <div className={`absolute bottom-16 md:bottom-6 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-2 md:gap-4 bg-white/80 backdrop-blur-xl border border-white/40 p-1 md:p-1.5 rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.12)] animate-in slide-in-from-bottom-8 duration-700 ${hiddenOnMobile ? 'hidden md:flex' : 'flex'}`}>

            {/* Show All Toggle */}
            <div className="flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-slate-50/50 rounded-full border border-slate-100/50">
                <span className="hidden sm:block text-[10px] font-black uppercase text-slate-400 tracking-widest">Show All</span>
                <button
            ...
            {/* Rotate Control */}
            <div className="flex flex-col gap-1 w-20 md:w-28 px-1 md:px-2">
